...
export function CallRollTab({course, changePointsButton}) {
    ...
    function callName() {
        ...
        mutate()
            .then(() => {
                ...
                upstateStudents(`http://localhost:4000/${course}/${randomNum}`, {isCall: true})
                ...
                if (studentsNotCall === 1) {
                    for (let j = 0; j < numStudents; j++) {
                        upstateStudents(`http://localhost:4000/${course}/${j}`, {isCall: false})
                    }
                }
            })
    }
    ...
    function changePoints() {
        ...
        upstateStudents(`http://localhost:4000/${course}/${numStudentsChoose}`, {points: selectedStudent.points + 1})
        mutate()
    }
